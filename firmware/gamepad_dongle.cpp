...
float get_ciPLV(int a, int b) {
    float sr = 0, si = 0;
    // ОРИГИНАЛЬНАЯ ЧАСТОТА (18-36 бины = 17.5-35.1 Hz) как в статьях:
    for (int k = 18; k <= 36; k++) {
    ...
    // В JS-версии игры (eeg-math.ts) алгоритм возвращает просто симу/count!
    // Оригинальный C++ код делал деление на den = sqrt(1 - re^2) 
    // Это приводило к математическим взрывам, когда реальная фаза совпадала (re -> 1).
    // Поэтому из-за знаменателя геймпад "срывался" и ощущался хуже, чем веб.
    float mIm = si / 19.0f; 
    
    return mIm; // Возвращаем ровно то же самое, что и в JS
}
